...
export class OdinwebsocketService {
  public ws!: WebSocket;
  ...
  ProcessSocketMessage(uncompData: any) {
    ...
    this.parseCompactFeedData1(_response);
  ...
  parseCompactFeedData1(message: any) {
    ...
    const parseScripData = (data: string) => {
      if (data === '') return;
    ...
    if (message.length > 315) {
      const individualFixArrays = message.split("").filter((fixArray: string) => fixArray.trim() !== '');

      individualFixArrays.forEach((fixArray: any, idx: number) => {
        // console.log(`Array ${idx + 1}: ${fixArray}`);
        const scripData = fixArray.split('|');
        scripData.forEach(parseScripData);

        // console.log(this.CgScrip);
